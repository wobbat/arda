...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, }, "x", function() awful.spawn("rofi -show run") end,
        { description = "open a terminal", group = "launcher" }), -- Standard program
...
-- Manage borders based on layout and number of clients
local function set_border(c)
    local s = awful.screen.focused()
    local layout = s.selected_tag and s.selected_tag.layout

    if layout == awful.layout.suit.max.fullscreen or c.maximized then
        -- Always hide borders in max.fullscreen or when maximized
        c.border_width = 0
    elseif #s.tiled_clients == 1 and not c.floating then
        -- Hide borders when only one tiled client in other layouts
        c.border_width = 0
        beautiful.useless_gap = 16
    else
        -- Show borders for other cases
...
tag.connect_signal("property::layout", function(t)
    ...
    for _, c in pairs(t:clients()) do
        if layout == awful.layout.suit.max.fullscreen then
            c.border_width = 0
            beautiful.useless_gap = 16
        elseif #t.screen.tiled_clients == 1 and not c.floating then
            -- Hide borders when only one tiled client in other layouts
            c.border_width = 0
            beautiful.useless_gap = 16
        else
